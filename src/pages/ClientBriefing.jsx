...
export default function ClientBriefing() {
  ...
  const { data: latestReport } = useQuery({
    ...
    queryFn: async () => {
      const reports = await base44.entities.WeeklyReport.filter({ projectId }, '-reportingPeriodEnd', 1);
      return reports[0];
    },
    enabled: !!projectId
  });
  ...
  const generateBriefing = async () => {
    ...
    const highlights = latestReport?.highlights
      ? latestReport.highlights.split('\n').filter(Boolean).map(h => `- ${h}`).join('\n')
      : '- No recent highlights available';
    const upcoming = latestReport?.nextWeekFocus
      ? latestReport.nextWeekFocus.split('\n').filter(Boolean).map(n => `- ${n}`).join('\n')
            ...
            {latestReport && (
              <div className="pt-4 border-t" style={{ borderColor: 'rgba(202, 220, 252, 0.1)' }}>
                ...
                <div className="text-sm" style={{ color: '#94A3B8' }}>
                  Period ending: {latestReport.reportingPeriodEnd ? new Date(latestReport.reportingPeriodEnd).toLocaleDateString() : 'N/A'}
                </div>
